...
def train(paramdict=None):
    ...
    for numiter in range(params['nbiter']):
        ...
        for epoch in range(params['start_epoch'], params['epochs']):
            ...
            for t in range(0, len(subset), params['batch_size']):

                batchStart = t
                batchEnd = min(t + params['batch_size'], len(subset))
                ...
                init = dataram['templatei'][batch - init_index] #改
                pre = dataram['template'][batch - pre_index]
                gt = dataram['templatei'][batch + gt_index - 1]

                # pdb.set_trace()
                temp = np.concatenate((init, pre, cur)) #该
