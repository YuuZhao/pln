...
for numpic in range(NBPICS):
    ...
    #inputsTensor, targetPattern = pics.generateTestInput(myparams)
    Ti = 1

    y = net.initialZeroState()
    hebb = net.initialZeroHebb()
    yout =y
    inputsTensor, targetPattern = pics.generateGTArr(myparams, Ti,yout,Tout=False)
    #net.zeroDiagAlpha()
   ...
   # yout = y
   # inputsTensor, targetPattern = pics.generateGTArr(myparams,Ti,yout,Tout=True)
    ...
    #output = y.data.cpu().numpy()[0][:-1]
    #print(output.shape)
    output = y.data.cpu().numpy()[0][:-1].reshape((imagesize, imagesize))
    pattern1 = inputsTensor.cpu().numpy()[0][0][:-1].reshape((imagesize, imagesize))
    pattern2 = inputsTensor.cpu().numpy()[myparams['prestime']+1][0][:-1].reshape((imagesize, imagesize))
    pattern3 = inputsTensor.cpu().numpy()[2*(myparams['prestime'])+1][0][:-1].reshape((imagesize, imagesize))
    ...
    #print("Pattern:")
    #print(pattern1,pattern1.shape)
    #print(pattern2)
    #print(pattern3)
    #print(blankedpattern)
